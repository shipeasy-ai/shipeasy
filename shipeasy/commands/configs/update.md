...
flow in the dashboard (`shipeasy configs draft` / `shipeasy configs
publish`).
...
3. CLI fallback:
   ```bash
   shipeasy configs update <name> --value '<json-value>'
   ```
...
4. Per-env staging (preferred for production-facing configs — lets you
   ...
   ```bash
   shipeasy configs draft   <name> --env staging --value '<json>'
   shipeasy configs publish <name> --env staging
   ```
