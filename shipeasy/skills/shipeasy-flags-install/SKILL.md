...
> and refresh the plugin: `/plugin marketplace update shipeasy` (Claude Code)
or re-run `npx skills add` on other hosts.

## Steps

1. Confirm the base onboarding is in place. If this fails, stop and run the
   `shipeasy-setup` skill first:
...
That's the whole install. The other two install sections are the
`shipeasy-ops-install` (feedback + production errors) and
`shipeasy-i18n-install` (translations) skills.
...
- the `shipeasy-experiments` skill — design + draft an A/B test (analyze the
  codebase, propose a variation point + success metric, instrument events, draft).
- the `shipeasy-metrics` skill — analyze + instrument + create a metric.
