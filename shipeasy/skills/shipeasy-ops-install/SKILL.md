...
This skill is a **thin, language-agnostic wrapper**. It does not embed
...
docs are the version-correct source of truth; this skill never goes stale
against a particular SDK's API.

Prereq: the `shipeasy-setup` skill already ran successfully and `.shipeasy`
exists at the repo root (base setup installs the SDK + the one `configure(...)`
boot this builds on). If it didn't, stop and run `shipeasy-setup` first.
...
- **Plugin (skills):** `/plugin marketplace update shipeasy` then
  `/plugin install shipeasy@shipeasy` (Claude Code), or re-run
  `npx skills add` on other hosts.
...
If the check fails, stop and run the `shipeasy-setup` skill first.
...
> separate module-enable subcommand; `shipeasy install <module>` is the only
> module enable path.
...
## 7. Skills ship with the plugin — do NOT hand-write them

The `shipeasy-ops` and `shipeasy-see` skills (and the rest of the platform)
ship in the **Shipeasy plugin**. Because this skill is running, the plugin (or
a skills-CLI install) is already in place and both skills are live — **there is
...
that base `shipeasy-setup` drops (`.claude/skills/shipeasy-onboarded/SKILL.md`),
which lists every install skill including this one.
...
Full grammar: the `shipeasy-see` skill and
...
- **"Wrap errors in see()":** invoke the `shipeasy-see` skill and sweep the codebase
