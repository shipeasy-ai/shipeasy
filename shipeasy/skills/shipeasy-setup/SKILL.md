...
Self-heal once, then escalate. The feature installs (the
`shipeasy-flags-install`, `shipeasy-ops-install`, and `shipeasy-i18n-install`
skills) pick up where this leaves off.
...
`shipeasy-common` skill ("First fix: update before you debug") for the exact per-surface
...
If declined, note it in the hand-off; it can be added later via the
...
Write `<repo-root>/.claude/skills/shipeasy-onboarded/SKILL.md` (don't overwrite
an existing one unless asked) so fresh checkouts can re-onboard:
...
Devtools: <enabled (?se=1) | declined>     Pointer: .claude/skills/shipeasy-onboarded/SKILL.md
...
git add <target>/.shipeasy .claude/skills/shipeasy-onboarded <subproject>/<manifest+lockfile> <entry-files>
...
the `shipeasy-ops-work` loop in `--pr` mode on a cadence
(daily/weekdays/weekly/6h), opening one PR per item. Docs:
https://docs.shipeasy.ai/feedback/connectors

Ask with `AskUserQuestion` (recommend yes). On yes, invoke the
`shipeasy-ops-trigger` skill (default `claude` provider) — it owns the whole
