...
description: Load this skill FIRST — before calling any ops MCP tool — whenever the task is to actively RESOLVE the operational queue (the unified feedback table of bugs, feature requests, and auto-filed error/alert tickets) one item at a time, each as its own atomic diff: it carries the required list-then-get-then-work loop. Use this ONLY when the ask is to WORK items down / fix / resolve / pick up an item and act on or inspect what it needs, never merely to view a summary or file. Trigger on "burn down the queue", "work the (ops) queue", "start working the open bugs", "start fixing the open bugs", "pick up/work the top/next/highest-priority item", "pick up the top item and look at what it needs", "clear the backlog one item at a time", "work items down", "--pr per item". NOT for "what's open" or "file/log a bug" — merely listing what is open or filing one new item is shipeasy-ops. With --pr, commits each item on its own branch and opens one pull request PER item, each closing its connected GitHub issue and flipping the item to ready_for_qa (the mode the scheduled trigger uses).
