...
/**
 * FS/AST source scanner for `shipeasy i18n scan` / `validate`. Walks `.tsx`/`.jsx`
 * files and surfaces translatable JSX text, string props, and existing `t("…")`
 * calls. CLI-strict — the whole fs/AST surface lives in the CLI (MCP never
 * touches the filesystem). Was previously imported from @shipeasy/mcp; vendored
 * here so the CLI owns it and has no MCP dependency.
 */
