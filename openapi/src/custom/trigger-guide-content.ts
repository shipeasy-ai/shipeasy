/**
 * Per-provider content for the `trigger guide` custom op — the authoritative
 * source of the "install the Shipeasy feedback trigger on <platform>" runbook
 * (also published at https://docs.shipeasy.ai/get-started/triggers). The
 * shipeasy-ops-trigger skill calls the op and follows this output instead of
 * shipping its own copy, so the walkthrough updates with the package, not the
 * plugin. Verified against vendor docs as of 2026-06-17 — see the caveat block.
 *
 * Editing note: sections are template literals with ` and ${ escaped.
 */

export type TriggerTier = "A" | "B" | "C";

export interface TriggerProviderMeta {
  tier: TriggerTier;
  scheduler: string;
  /** Whether Shipeasy's Worker can fire this provider as a connector. */
  shipeasyFireable: boolean;
}

export const TRIGGER_PROMPT = `Whatever the provider, the scheduled run executes this **trigger prompt** (the
...
INSTALL.md (https://docs.shipeasy.ai/get-started/agents): the plugin install for \`claude\`/\`copilot\`, or
...
export const TRIGGER_AUTH_SAFETY = `## Auth & safety (every provider)
...
export const TRIGGER_CAVEAT = `## What's confirmed vs. moving
...
export const TRIGGER_NOT_FIREABLE_NOTE = `- **\`windsurf\` / \`cline\` / \`openclaw\` / \`opencode\` / \`continue\`** — ❌ scheduled
...
export const TRIGGER_PROVIDER_META: Record<string, TriggerProviderMeta> = {
  claude: { tier: "A", scheduler: "Claude Code scheduled routine (Anthropic cloud; no GitHub Actions)", shipeasyFireable: true },
  cursor: { tier: "A", scheduler: "Cursor Automations / Cloud Agents API (vendor cloud cron)", shipeasyFireable: true },
  copilot: { tier: "A", scheduler: "Copilot automations / cloud agent (Agents tab, or Actions-driven)", shipeasyFireable: true },
  windsurf: { tier: "A", scheduler: "Devin Scheduled Sessions (vendor cloud cron)", shipeasyFireable: false },
  cline: { tier: "B", scheduler: "`cline schedule` \u2014 local daemon; host must stay awake", shipeasyFireable: false },
  openclaw: { tier: "B", scheduler: "`openclaw cron` \u2014 local gateway daemon; host must stay awake", shipeasyFireable: false },
  codex: { tier: "C", scheduler: "GitHub Actions `schedule:` cron \u2192 `codex cloud exec` (Codex Cloud task)", shipeasyFireable: false },
  opencode: { tier: "C", scheduler: "`opencode run` headless + external cron (system cron or GitHub Actions)", shipeasyFireable: false },
  continue: { tier: "C", scheduler: "`cn -p --auto` headless + external cron", shipeasyFireable: false },
  gemini: { tier: "C", scheduler: "Gemini CLI headless + external cron, or Jules (Shipeasy-fireable)", shipeasyFireable: true },
};

export const TRIGGER_PROVIDER_SECTIONS: Record<string, string> = {
  claude: `### \`claude\` — Claude Code scheduled routine
...
the \`shipeasy-ops-trigger\` skill with \`--provider claude\`.
...
allowlist (Custom network).`,
  ...
  cursor: `### \`cursor\` — Cursor Automations / Cloud Agents API
...
- **Note:** Automations always run in Max Mode (billed on cloud-agent usage).`,
  ...
  copilot: `### \`copilot\` — Copilot automations / cloud agent
...
Requests" permission** (and a Copilot subscription).`,
  ...
  windsurf: `### \`windsurf\` — Devin Scheduled Sessions
...
(service-user token, \`cog_\`/\`apk_\`).`,
  ...
  cline: `The schedule is built in, but runs on a daemon on your machine — keep the host
awake (an external-cron Tier-C setup is the always-on alternative).
...
shelling out to \`git\` + \`gh\` (needs \`gh\` authed in the daemon's env).`,
  ...
  openclaw: `The schedule is built in, but runs on a daemon on your machine — keep the host
awake (an external-cron Tier-C setup is the always-on alternative).
...
configured with an API key, not a login.`,
  ...
  codex: `### \`codex\` — confirmed flow: GitHub Actions cron → \`codex cloud exec\` → Codex Cloud
...
**Do the work upfront — don't just hand the user a checklist.** The
\`shipeasy-ops-trigger\` skill (\`--provider codex\`) should: mint the \`ops\` key, **write the workflow file** with
...
> env page is the single config surface.`,
  ...
  opencode: `### \`opencode\` — headless run + external scheduler
...
> mechanism.`,
  ...
  continue: `### \`continue\` — headless run + external scheduler
...
> mechanism.`,
  ...
  gemini: `### \`gemini\` — headless run + external scheduler
...
export const TRIGGER_FIREABLE_NOTES: Record<string, string> = {
  claude: `- **\`claude\`** — ✅ live. \`POST …/v1/claude_code/routines/<id>/fire\` with a
  ...
  cursor: `- **\`cursor\`** — ✅ live. \`POST https://api.cursor.com/v1/agents\` with
  ...
  copilot: `- **\`copilot\`** — ✅ live (with an auth caveat). \`POST
  ...
  **"Agent tasks"** repo permission (read+write) for a Copilot-licensed account.
  ...
  codex: `- **\`codex\`** — ❌ no fire endpoint **for our Worker**. A real cloud task IS
